...
class AlatLukis {
    ...
    public void updateInfo(double harga) {
        if (harga >= 0) {
            this.harga = harga;
        }
    }

    public void updateInfo(String nama, int stok, double harga) {
        updateInfo(nama);
        updateInfo(stok);
        updateInfo(harga);
    }
    ...
    public static void updateAlatLukis() {
        ...
        if (!daftarAlatLukis.isEmpty()) {
            ...
            if (index >= 0 && index < daftarAlatLukis.size()) {
                ...
                System.out.print("Masukkan harga baru (ketik -1 jika tidak ingin mengubah): ");
                double harga = scanner.nextDouble();
                scanner.nextLine();
                if (harga != -1) {
                    alatLukis.updateInfo(harga); 
                }
